  ...
  const getProductsList = async () => {
    try {
      ...
      if (Array.isArray(data) && data.length) {
        const updatedProducts = data.map((product: ProductType) => {
          return {
            ...product,
        ...
        console.log('No products received or data format is incorrect.');
        setProductList([]); // Set an empty array to indicate no products
      }
    } catch (error) {
      console.error('Failed to get products list!', error);
      setProductList([]); // Set an empty array on error as well
    }
  };
